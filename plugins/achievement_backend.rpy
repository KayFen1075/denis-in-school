        ...
        def timestamp(self):
            ...
            if self.has():
                try:
                    ...
                    return ""
                return __("Разблокированно ") + ts.strftime(__(
            ...
            if not has_achievement:
                ...
                if not self.hidden:
                    if self.hide_description:
                        OneDiscordMessage("# Достижение! 🏅\n{0} открыл достижение **{1}**".format(persistent.user_name, self.name, self.description))
                    else:
                        OneDiscordMessage("# Достижение! 🏅\n{0} открыл достижение **{1}**\n```\n{2}```".format(persistent.user_name, self.name, self.description))
                OneDiscordMessage("# Достижение! 🏅\n{0} открыл достижение! `???`".format(persistent.user_name, self.name, self.description))
